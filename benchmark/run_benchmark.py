...
def load(name):
    ...
    return (
        [json.loads(l) for l in open(p, encoding="utf-8")] if os.path.exists(p) else []
    )
...
results = {
    ...
    "recall": {
        "overall": round(overall_recall, 4),
        "caught": tot_caught,
        "total": tot_pos,
        "by_type": {
            k: {"caught": v[0], "total": v[1], "recall": round(v[0] / v[1], 4)}
            for k, v in sorted(recall_by_type.items())
        },
    },
    "precision_on_negatives": {
        "overall": round(overall_prec, 4),
        "correctly_ignored": prec[0],
        "total": prec[1],
    },
...
json.dump(
    results,
    open(os.path.join(HERE, "benchmark_results.json"), "w"),
    indent=2,
    ensure_ascii=False,
)
...
L.append(
    "_Generated on-box (Mac mini ornith-9b + nomic) · zero egress · "
...
L.append(
    f"**Corpus:** {len(rows)} rows — {tot_pos} valid positives, {prec[1]} look-alike negatives\n"
)
L.append("\n## Headline\n")
L.append("| Axis | Score | |")
L.append("|---|---|---|")
L.append(
    f"| **Recall** (valid IDs caught) | **{overall_recall:.1%}** | {tot_caught}/{tot_pos} |"
)
L.append(
    f"| **Precision** (look-alikes ignored) | **{overall_prec:.1%}** | {prec[0]}/{prec[1]} |"
)
L.append(
    f"| Residue net caught of the recall misses | {residue_saved}/{len(misses)} | safety tier |"
)
...
if not leaks:
    ...
    for lk in leaks:
        L.append(
            f"| {lk['type']} | {lk['variant']} | {lk['detected']} | `{lk['text'][:48]}` |"
        )
...
print(
    f"recall {overall_recall:.1%} ({tot_caught}/{tot_pos})  "
    f"precision {overall_prec:.1%} ({prec[0]}/{prec[1]})"
)
print(
    f"recall misses: {len(misses)} (residue caught {residue_saved})  precision leaks: {len(leaks)}"
)
