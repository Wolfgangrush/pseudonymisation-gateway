...
def mint_aadhaar():
    for _ in range(12):
        body = str(random.randint(2, 9)) + "".join(
    ...
    add(
        ...
        "lookalike",
        "leading-1",
        "first digit 1 — UIDAI never issues",
    )
    add(
        "847362910000",
        "AADHAAR",
        ...
        "lookalike",
        "bad-verhoeff",
        "valid shape, wrong check digit",
    )
    add(
        "Invoice 2024062800031",
        "AADHAAR",
        False,
        "lookalike",
        "invoice-13d",
        "13-digit invoice, not Aadhaar",
    )
    add(
        "UPI Ref 234567890124",
        "AADHAAR",
        False,
        "lookalike",
        "upi-ref",
        "12-digit UPI ref, bad checksum",
    )
...
def mint_pan():
    ...
    for _ in range(10):
        pan = (
            "".join(random.choice(L) for _ in range(3))
            + random.choice(HOLDER)
            + random.choice(L)
    ...
    add(
        ...
        "lookalike",
        "lowercase",
        "lowercase — gateway PAN_RE is [A-Z] only",
    )
...
def mint_gstin():
    ...
    add(
        ...
        "lookalike",
        "bad-checksum",
        "classic shaped-but-invalid GSTIN",
    )
    # invalid state code 40
    bad_first = "40ABCPE1234F1Z"
    add(
        bad_first + gstin_check(bad_first),
        "GSTIN",
        False,
        "lookalike",
        "bad-state",
    ...
    for cc, blen in (("GB", 18), ("AE", 19), ("DE", 18)):
        for _ in range(4):
            ...
            add(
                ...
                "valid",
                f"{cc}-spaced",
                "grouped-4 real-world print form",
            )
    add(
        "GB00BARC20031820000000",
        "IBAN",
        False,
        "lookalike",
        "bad-mod97",
        "GB-shaped, check digits don't reconcile",
    )
...
def mint_nric():
    ...
    add(
        ...
        "valid",
        "M-series",
        "M accepted on structure (gateway stance)",
    )
    # wrong check letter
    d7 = "1234567"
    good = nric_check("S", d7)
...
def mint_itin():
    for _ in range(8):
        g = random.choice(
            list(range(70, 89)) + list(range(90, 93)) + list(range(94, 100))
        )
    ...
    add(
        ...
        "lookalike",
        "bad-group",
        "group 89 outside assigned ranges",
    )
    add(
        "812-70-0000",
        "ITIN",
...
def mint_misc():
    ...
    for t, note in [
        ("₹5,000", "plain"),
        ("-₹5,000", "negative"),
        ("₹-5,000", "inner-negative"),
        ("(₹5,000)", "accounting-parens"),
        ("Rs. -5,000", "rs-negative"),
...
for r in ROWS:
    ...
    if r["type"] == "AADHAAR":
        ...
        ok = chk(raw)
    if ok != r["expect_detect"] and not (
        r["type"] == "NRIC" and r["variant"] == "M-series"
    ):
